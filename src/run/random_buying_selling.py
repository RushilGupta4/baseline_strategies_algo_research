...
from strategies.random_buying_selling import RandomBuyingSelling
from constants import TransactionSide

TIME_SPAN = 30
BASE_PATH = os.path.join("data", "zerodha_d1")
OUTPUT_BASE_PATH = os.path.join("output", f"random_buying_selling")
...
def run_for_symbol(file_name):
    ...
    data = utils.get_data_from_path(file_path, TIME_SPAN)

    strategy = RandomBuyingSelling(symbol=symbol_name, data=data)
